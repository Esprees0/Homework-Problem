    ...
    def display_seats(self):
        ...
        while current:
            ...
            if current.data.is_reserved:
                print(f"Booked by: {current.data.booked_by} student code: {code_booked}")
                
            current = current.next

    def reserve_seat(self, seat_number, booked_by, code_booked):
        current = self.seat_list.head
        while current:
            if current.data.seat_number == seat_number and not current.data.is_reserved:
                ...
                current.data.code_booked = code_booked
                print(f"Seat {seat_number} reserved by {booked_by} student code {code_booked}")
                return
            current = current.next
        ...
        while current:
            if current.data.is_reserved:
                reserved_seats.append({
                    ...
                    'student code': current.data.code_booked
                })
            current = current.next
...
while True:
    ...
    if choice == 1:
        ...
        booked_by = str(input("Enter student name: "))
        code_booked = str(input("Enter student code: "))
        theater.reserve_seat(seat_number, booked_by ,code_booked)
    elif choice == 4:
